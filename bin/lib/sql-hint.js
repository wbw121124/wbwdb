// [FIX] Advanced SQL Hint Engine - Brigadier-style intelligent suggestions
...
// Operators
const OPERATORS = ['=', '!=', '<>', '<', '>', '<=', '>=', 'LIKE', 'ILIKE', 'IN', 'NOT IN', 'IS', 'IS NOT', 'BETWEEN', 'EXISTS'];

// Join types
const JOIN_TYPES = ['INNER', 'LEFT', 'RIGHT', 'FULL', 'CROSS', 'NATURAL'];
...
export class SQLHintEngine {
	...
	getSuggestions(input, cursorPos) {
		const context = this.analyzeContext(input, cursorPos);
	...
	getNextKeywordSuggestions(lastKeyword, tokens) {
	...
	analyzeContext(input, cursorPos) {
